        ...
        if (this.options.search) {
            ...
            if (isIEBrowser()) {
                $search.off('mouseup').on('mouseup', function (event) {
                	if (that.options.searchOnEnterKey) return;
                    clearTimeout(timeoutId); // doesn't matter if it's 0
                    timeoutId = setTimeout(function () {
                        that.onSearch(event);
                    }, that.options.searchTimeOut);
...
!function ($) {
    ...
    BootstrapTable.prototype.init = function () {
        ...
        /**
         * 本来是这样的，resize和orientationchange都会触发重置窗口的事件，但存在输入框时，屏幕键盘的弹出会触发resize事件，导致后面调用resetView收起屏幕键盘
         * 出现的现象就是屏幕键盘弹出来一下就自动收回去，无法输入。这里把resize事件去掉，不好的地方是浏览器尺寸的变化无法再触发移动视图的改变，但对比移动端下无法输入，这个显得很次要了
         */
